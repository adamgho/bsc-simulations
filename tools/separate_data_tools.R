...
# Read directory to save files in
default_dir <- scan('run_scripts/data_dir.txt',
                    what = 'char',
                    quiet = TRUE)

### This file contains alternate versions of the simulation functions, where the
### data comes from two actual separate experiments, rather than just one
### experiments with permutation to emulate a second experiment.

### The functions have the same names, and the results produced will be saved in
### files named exactly the same, to save a lot of extra work in development of
### this code. To distinguish, you should save them in a specific subdirectory
### of the data directory. One has to run the processing functions (to
### calculate AUC) separately in this subdirectory, and load the results in a
### second tibble, since it would otherwise be indistinguishable from regular
### alltargets from the permutation setup.
...
sim_alltargets <- function(DAG_data,
  ...
  ## Simulate from the separate tilde-experiment
  sim_list_tilde <- lapply(1:num_interv, interv_sim)
...
# data/separate/alltargets_[n_obs_each]_[num_interv]_sdw[shift_noise_sd]_sdh[sd_hiddens]
...
sim_alltargets_datasets <- function(DAG_list, n_obs_each, num_interv,
                                    ...
                                    dir = default_dir, max_MB_per_file = 500) {
